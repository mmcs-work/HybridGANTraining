...
hdlr = logging.FileHandler('./dcgan.log')
...
for epoch in range(opt.niter):
    ...
    mean_errD = np.array(errDs, dtype=np.float64).mean()
    mean_errG = np.array(errGs, dtype=np.float64).mean()
    logger.info(f'err_D: {mean_errD}, err_G: {mean_errG}')
